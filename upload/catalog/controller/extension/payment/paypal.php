...
class ControllerExtensionPaymentPayPal extends Controller {
			...
			if ($page_code != 'checkout') {
				...
				if ($paypal_order_info && !$this->error) {
					...
					if ($this->customer->isLogged()) {
						...
						$this->session->data['guest']['telephone'] = ($paypal_order_info['payer']['phone']['phone_number']['national_number'] ?? '');
					...
					if (($payment_type == 'googlepay_button') && !empty($this->request->post['payment_data'])) {
						...
						$this->session->data['guest']['firstname'] = ($payment_firstname ?? '');
						$this->session->data['guest']['lastname'] = ($payment_lastname ?? '');
						$this->session->data['guest']['email'] = ($payment_data['email'] ?? '');
						$this->session->data['guest']['telephone'] = ($payment_data['paymentMethodData']['info']['billingAddress']['phoneNumber'] ?? '');

						$this->session->data['payment_address']['firstname'] = ($shipping_firstname ?? '');
						$this->session->data['payment_address']['lastname'] = ($shipping_lastname ?? '');
						$this->session->data['payment_address']['address_1'] = ($payment_data['paymentMethodData']['info']['billingAddress']['address1'] ?? '');
						$this->session->data['payment_address']['city'] = ($payment_data['paymentMethodData']['info']['billingAddress']['locality'] ?? '');
						$this->session->data['payment_address']['postcode'] = ($payment_data['paymentMethodData']['info']['billingAddress']['postalCode'] ?? '');
						...
						if ($this->cart->hasShipping()) {
							...
							$this->session->data['shipping_address']['firstname'] = ($shipping_firstname ?? '');
							$this->session->data['shipping_address']['lastname'] = ($shipping_lastname ?? '');
							$this->session->data['shipping_address']['address_1'] = ($payment_data['shippingAddress']['address1'] ?? '');
							$this->session->data['shipping_address']['city'] = ($payment_data['shippingAddress']['locality'] ?? '');
							$this->session->data['shipping_address']['postcode'] = ($payment_data['shippingAddress']['postalCode'] ?? '');
					...
					if (($payment_type == 'applepay_button') && !empty($this->request->post['payment_data'])) {
						...
						$this->session->data['guest']['firstname'] = ($payment_data['billingContact']['givenName'] ?? '');
						$this->session->data['guest']['lastname'] = ($payment_data['billingContact']['familyName'] ?? '');
						$this->session->data['guest']['email'] = ($payment_data['shippingContact']['emailAddress'] ?? '');
						$this->session->data['guest']['telephone'] = ($payment_data['shippingContact']['phoneNumber'] ?? '');

						$this->session->data['payment_address']['firstname'] = ($payment_data['billingContact']['givenName'] ?? '');
						$this->session->data['payment_address']['lastname'] = ($payment_data['billingContact']['familyName'] ?? '');
						...
						$this->session->data['payment_address']['city'] = ($payment_data['billingContact']['locality'] ?? '');
						$this->session->data['payment_address']['postcode'] = ($payment_data['billingContact']['postalCode'] ?? '');
						...
						if ($this->cart->hasShipping()) {
							$this->session->data['shipping_address']['firstname'] = ($payment_data['shippingContact']['givenName'] ?? '');
							$this->session->data['shipping_address']['lastname'] = ($payment_data['shippingContact']['familyName'] ?? '');
							...
							$this->session->data['shipping_address']['city'] = ($payment_data['shippingContact']['locality'] ?? '');
							$this->session->data['shipping_address']['postcode'] = ($payment_data['shippingContact']['postalCode'] ?? '');
	...
	public function confirmOrder() {
		...
		if ($this->cart->hasShipping()) {
			...
			$data['shipping_address'] = $this->session->data['shipping_address'] ?? array();
		...
		$data['guest'] = $this->session->data['guest'] ?? array();
		$data['payment_address'] = $this->session->data['payment_address'] ?? array();
	...
	public function completeOrder() {
		...
		if (isset($this->session->data['paypal_order_id']) || isset($this->session->data['paypal_payment_token'])) {
			...
			$order_data['payment_custom_field'] = ($this->session->data['payment_address']['custom_field'] ?? array());
			...
			if ($this->cart->hasShipping()) {
				...
				$order_data['shipping_custom_field'] = ($this->session->data['shipping_address']['custom_field'] ?? array());
			...
			if (isset($this->session->data['paypal_order_id'])) {
				...
				if ($paypal_order_info && !$this->error) {
					$shipping_info_name = ($paypal_order_info['purchase_units'][0]['shipping']['name'] ?? array());
					$shipping_info_address = ($paypal_order_info['purchase_units'][0]['shipping']['address'] ?? array());
				}
	...
	public function fastlaneShipping() {
		...
		if (!empty($this->request->post['authentication_state']) && ($this->request->post['authentication_state'] == 'succeeded') && !empty($this->request->post['profile_data'])) {
			...
			$data['guest']['telephone'] = ($profile_data['shippingAddress']['phoneNumber']['nationalNumber'] ?? '');

			$data['shipping_address']['firstname'] = ($profile_data['shippingAddress']['name']['firstName'] ?? '');
			$data['shipping_address']['lastname'] = ($profile_data['shippingAddress']['name']['lastName'] ?? '');
			$data['shipping_address']['address_1'] = ($profile_data['shippingAddress']['address']['addressLine1'] ?? '');
			$data['shipping_address']['address_2'] = ($profile_data['shippingAddress']['address']['addressLine2'] ?? '');
			$data['shipping_address']['city'] = ($profile_data['shippingAddress']['address']['adminArea2'] ?? '');
			$data['shipping_address']['postcode'] = ($profile_data['shippingAddress']['address']['postalCode'] ?? '');
			$data['shipping_address']['country'] = ($profile_data['shippingAddress']['address']['countryCode'] ?? '');
			$data['shipping_address']['zone'] = ($profile_data['shippingAddress']['address']['adminArea1'] ?? '');
	...
	public function confirmFastlanePayment() {
		...
		if (!empty($this->request->post['payment_token'])) {
			...
			if (!empty($this->request->post['payment_source'])) {
				...
				$this->session->data['payment_address']['firstname'] = ($payment_firstname ?? '');
				$this->session->data['payment_address']['lastname'] = ($payment_lastname ?? '');
				$this->session->data['payment_address']['company'] = '';
				$this->session->data['payment_address']['address_1'] = ($payment_source['card']['billingAddress']['streetAddress'] ?? '');
				$this->session->data['payment_address']['address_2'] = ($payment_source['card']['billingAddress']['extendedAddress'] ?? '');
				$this->session->data['payment_address']['city'] = ($payment_source['card']['billingAddress']['locality'] ?? '');
				$this->session->data['payment_address']['postcode'] = ($payment_source['card']['billingAddress']['postalCode'] ?? '');
				$this->session->data['payment_address']['country'] = '';
				$this->session->data['payment_address']['country_id'] = 0;
				$this->session->data['payment_address']['address_format'] = '';
				$this->session->data['payment_address']['zone'] = '';
			...
			if (hash_equals($setting['general']['webhook_token'], $this->request->get['webhook_token']) && !empty($webhook_info['id']) && !empty($webhook_info['event_type'])) {
				...
				if (($webhook_event['event_type'] == 'VAULT.PAYMENT-TOKEN.CREATED') && !empty($webhook_info['resource']['id']) && !empty($webhook_info['resource']['customer']['id']) && !empty($webhook_event['resource']['metadata']['order_id']) && !$errors) {
					...
					if ($paypal_order_info) {
						...
						$card_last_digits = ($webhook_event['resource']['payment_source']['card']['last_digits'] ?? '');
						$card_expiry = ($webhook_event['resource']['payment_source']['card']['expiry'] ?? '');
