...
function check_requirements() {
	...
	if (version_compare(phpversion(), '8.0', '<')) {
		$error = 'Warning: You need to use PHP 8.0+ or above for OpenCart to work!';
	}
...
function setup_db($data) {
	...
	if ($lines) {
		...
		$db->query("INSERT INTO `" . $db_prefix . "user` SET `user_id` = '1', `user_group_id` = '1', `username` = '" . $db->escape($data['username']) . "', `password` = '" . $db->escape(password_hash(html_entity_decode($data['password'], ENT_QUOTES, 'UTF-8'), PASSWORD_DEFAULT)) . "', `firstname` = 'John', `lastname` = 'Doe', `email` = '" . $db->escape($data['email']) . "', `status` = '1', `date_added` = NOW()");
