...
class ControllerExtensionPaymentSquareup extends Controller {
	...
	protected function validate() {
		...
		if (!empty($this->request->post['payment_squareup_enable_sandbox'])) {
			...
			if ($this->config->get('payment_squareup_sandbox_locations') && isset($this->request->post['payment_squareup_sandbox_location_id']) && !in_array($this->request->post['payment_squareup_sandbox_location_id'], array_map(fn ($location) => $location['id'], $this->config->get('payment_squareup_sandbox_locations')))) {
			...
			if ($this->config->get('payment_squareup_locations') && isset($this->request->post['payment_squareup_location_id']) && !in_array($this->request->post['payment_squareup_location_id'], array_map(fn ($location) => $location['id'], $this->config->get('payment_squareup_locations')))) {
				$this->error['location'] = $this->language->get('error_no_location_selected');
			}
		}
